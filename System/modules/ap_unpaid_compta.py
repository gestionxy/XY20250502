...
def ap_unpaid_query_compta():
    ...
    df = df[~df['公司名称'].isin(['SLEEMAN', 'Arc-en-ciel','Ferme vallee verte','MS TRT'])]
   ...
   # -------------------------------
    ...
    mask_no_posting_date_1 = df['银行对账日期'].isna()
    df.loc[mask_no_posting_date_1, '实际支付金额'] = 0

    # 银行对账日期存在（非空）
    mask_bank_date_exists_1 = df['银行对账日期'].notna()

    # 银行对账日期不在发票日期范围内
    mask_bank_not_in_range_1 = ~(
        (df['银行对账日期'] >= pd.to_datetime(start_date)) &
        (df['银行对账日期'] <= pd.to_datetime(end_date))
    )

    # 最终筛选：银行对账日期存在 且 不在发票日期范围
    mask_final_1 = mask_bank_date_exists_1 & mask_bank_not_in_range_1

    # 更新“实际支付金额”为0
    df.loc[mask_final_1, '实际支付金额'] = 0
    ...
    for dept, df_dept in filtered.groupby('部门'):
        ...
        for company, df_comp in df_dept.groupby('公司名称'):
            # 拼接当前公司所有明细数据，只保留指定列
            final = pd.concat([final, df_comp[['部门', '公司名称', '发票号', '发票日期','开支票日期','银行对账日期', '发票金额', '付款支票号','实际支付金额', '应付未付差额','TPS','TVQ']]])
        ...
        subtotal['发票日期'] = pd.NaT
        subtotal['开支票日期'] = pd.NaT       # 小计行不设日期，用 pd.NaT 保持类型一致
    ...
    total['付款支票号'] = ''
    total['发票日期'] = pd.NaT
    total['开支票日期'] = pd.NaT         # 同样用 NaT 表示“无日期”
    total['银行对账日期'] = pd.NaT
    ...
    final['开支票日期'] = final['开支票日期'].apply(
        lambda d: d.strftime('%Y-%m-%d') if pd.notnull(d) else ''
    )

    # 步骤 4：按指定字段顺序重新排列列，确保前端显示或导出一致
    final = final[['部门', '公司名称', '发票号', '发票日期','开支票日期','银行对账日期', '发票金额','付款支票号', '实际支付金额', '应付未付差额','TPS','TVQ']]
